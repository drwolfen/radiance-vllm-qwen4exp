#!/usr/bin/env python3
"""Standard vLLM Extended Benchmark Suite for Qwen3.8-Flash-Next (qwen4exp).
...
async def send_streaming_request(url, model, messages, max_tokens, temperature=0.0, tools=None):
    ...
    def _run():
        ...
        output_tokens = usage_tokens if usage_tokens is not None else len(token_times)
...
async def main():
    ...
    print("  Hardware: 2x AMD Radeon AI PRO R9700 (gfx1201) · ROCm 7.14 / R9V · PyTorch 2.12.1")
    print("=========================================================================================")
    ...
    for _ in range(60):
        ...
        print("Server not responding after 300s. Exiting.")
        return
    ...
    for c in [1, 2, 4, 8, 16]:
        ...
        if res:
            ...
            print(row_fmt.format(
                f"{res['concurrency']} Stream{'s' if res['concurrency'] > 1 else ''}",
                f"{res['tok_per_sec']:.1f} tok/s",
                f"{res['req_per_sec']:.2f} req/s",
