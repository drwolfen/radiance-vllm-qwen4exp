...
def test_rdna4_qsa_strided_dispatch_is_opt_in(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(qsa_ops, "on_rdna4", lambda: True)
    monkeypatch.delenv(qsa_ops._RDNA4_QSA_STRIDED_ENV, raising=False)
    assert not qsa_ops._rdna4_qsa_strided_enabled()

    monkeypatch.setenv(qsa_ops._RDNA4_QSA_STRIDED_ENV, "1")
    assert qsa_ops._rdna4_qsa_strided_enabled()

    monkeypatch.setenv(qsa_ops._RDNA4_QSA_STRIDED_ENV, "0")
    assert not qsa_ops._rdna4_qsa_strided_enabled()

    monkeypatch.setattr(qsa_ops, "on_rdna4", lambda: False)
    monkeypatch.setenv(qsa_ops._RDNA4_QSA_STRIDED_ENV, "1")
    assert not qsa_ops._rdna4_qsa_strided_enabled()

    assert "PROGRAM_STRIDE" in qsa_ops._qsa_mqa_paged_strided_kernel.arg_names


def test_rdna4_qsa_strided_grid_and_mapping() -> None:
    rows = 1
    capacity_columns = 32_800

    assert qsa_ops._rdna4_qsa_strided_grid(rows, capacity_columns) == (rows, 128)
    assert qsa_ops._rdna4_qsa_strided_grid(rows, 64) == (rows, 2)

    short_num_tiles = math.ceil(66 / qsa_ops._QSA_MQA_BLOCK_N)
    short_mapping = [
        list(range(program, short_num_tiles, qsa_ops._RDNA4_QSA_STRIDED_PROGRAMS))
        for program in range(qsa_ops._RDNA4_QSA_STRIDED_PROGRAMS)
    ]
    assert short_mapping[:3] == [[0], [1], [2]]
    assert not any(short_mapping[3:])

    full_num_tiles = 32_768 // qsa_ops._QSA_MQA_BLOCK_N
    full_mapping = [
        list(range(program, full_num_tiles, qsa_ops._RDNA4_QSA_STRIDED_PROGRAMS))
        for program in range(qsa_ops._RDNA4_QSA_STRIDED_PROGRAMS)
    ]
    assert all(len(tiles) == 8 for tiles in full_mapping)
    assert sorted(tile for tiles in full_mapping for tile in tiles) == list(
        range(full_num_tiles)
    )


def test_rdna4_qsa_strided_scalar_mapping_matches_reference() -> None:
    torch.manual_seed(7)
    num_columns = 66
    query = torch.randn((4, 128), dtype=torch.bfloat16)
    keys = torch.randn((num_columns, 128), dtype=torch.bfloat16)
    divisor = math.sqrt(query.shape[-1])

    expected = torch.zeros(num_columns, dtype=torch.float32)
    for head in range(query.shape[0]):
        dots = torch.sum(keys.float() * query[head].float(), dim=1)
        expected += torch.relu(dots)
    expected /= divisor

    actual = torch.empty_like(expected)
    num_tiles = math.ceil(num_columns / qsa_ops._QSA_MQA_BLOCK_N)
    for program in range(qsa_ops._RDNA4_QSA_STRIDED_PROGRAMS):
        for tile in range(
            program,
            num_tiles,
            qsa_ops._RDNA4_QSA_STRIDED_PROGRAMS,
        ):
            start = tile * qsa_ops._QSA_MQA_BLOCK_N
            end = min(start + qsa_ops._QSA_MQA_BLOCK_N, num_columns)
            score = torch.zeros(end - start, dtype=torch.float32)
            for head in range(query.shape[0]):
                dots = torch.sum(keys[start:end].float() * query[head].float(), dim=1)
                score += torch.relu(dots)
            actual[start:end] = score / divisor
