...
from vllm.models.qwen4_exp.common.mtp import Qwen4ExpMTPMultimodalMixin
...
class _RecordingTextEmbedder(torch.nn.Module):
...
@pytest.fixture
def qwen4_exp_mtp_cls():
    class _BareQwen4ExpMTP(torch.nn.Module, Qwen4ExpMTPMultimodalMixin):
        pass

    return _BareQwen4ExpMTP


def _bare_qwen4_exp_mtp(qwen4_exp_mtp_cls):
    model = qwen4_exp_mtp_cls()
    model.model = _RecordingTextEmbedder()
    return model


def test_qwen4_exp_mtp_declares_external_multimodal_embedding_support(
    qwen4_exp_mtp_cls,
) -> None:
    assert supports_multimodal_embeddings(qwen4_exp_mtp_cls)
    assert not supports_multimodal(qwen4_exp_mtp_cls)


def test_qwen4_exp_mtp_multimodal_embedding_signature(qwen4_exp_mtp_cls) -> None:
    parameters = inspect.signature(qwen4_exp_mtp_cls.embed_input_ids).parameters
    ...
    assert parameters["is_multimodal"].kind is inspect.Parameter.KEYWORD_ONLY


def test_qwen4_exp_mtp_maps_external_embeddings_and_masks_oov_ids(
    qwen4_exp_mtp_cls,
) -> None:
    model = _bare_qwen4_exp_mtp(qwen4_exp_mtp_cls)
    input_ids = torch.tensor([4, 999, 5, 998])
    is_multimodal = torch.tensor([False, True, False, True])
    multimodal_embeddings = torch.tensor([[10.0, 11.0], [12.0, 13.0]])
    ...
    torch.testing.assert_close(model.model.input_ids, torch.tensor([4, 0, 5, 0]))
    torch.testing.assert_close(
        actual,
        torch.tensor([[4.0, 4.5], [10.0, 11.0], [5.0, 5.5], [12.0, 13.0]]),
    )


@pytest.mark.parametrize(
    ("is_multimodal", "error", "match"),
    [
        (None, ValueError, "requires `is_multimodal`"),
        (torch.tensor([0, 1]), TypeError, "must be a boolean tensor"),
        (
            torch.tensor([[False, True]]),
            ValueError,
            "must have the same shape as input_ids",
        ),
        (
            torch.tensor([False, False]),
            ValueError,
            "multimodal tokens to 0 placeholders",
        ),
    ],
)
def test_qwen4_exp_mtp_rejects_malformed_multimodal_masks(
    qwen4_exp_mtp_cls,
