...
def test_qsa_compressed_cache_uses_generic_tokens_per_state() -> None:
...
def test_qsa_side_cache_binds_canonical_view(layout: str) -> None:
    cache = qsa_cache.QSACompressedKeyCache(
        head_size=64,
        dtype=torch.bfloat16,
        cache_config=SimpleNamespace(block_size=64),
        prefix=f"compressed.{layout}",
        vllm_config=SimpleNamespace(
            compilation_config=SimpleNamespace(static_forward_context={})
        ),
        compress_ratio=4,
    )
    canonical = torch.arange(3 * 16 * 64, dtype=torch.bfloat16).reshape(3, 16, 1, 64)
    raw = canonical if layout == "LBNHC" else canonical.transpose(1, 2)

    cache.bind_kv_cache(raw)

    assert cache.kv_cache.shape == (3, 16, 1, 64)
    assert cache.kv_cache.data_ptr() == raw.data_ptr()
    assert torch.equal(cache.kv_cache, canonical)
