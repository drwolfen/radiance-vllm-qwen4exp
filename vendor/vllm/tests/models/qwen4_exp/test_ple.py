...
from vllm.models.qwen4_exp.amd import ple_layer as amd_ple_layer_module
...
def test_rocm_ngram_embedding_supports_cpu_offload(monkeypatch) -> None:
    embedding_cls = amd_ple_layer_module.Qwen4ExpNGramEmbedding
    assert issubclass(embedding_cls, PleOffloadLayer)

    module = embedding_cls.__new__(embedding_cls)
    nn.Module.__init__(module)
    ...
    module.positions_buffer = torch.arange(4, dtype=torch.int64)
    module.padded_buffer = torch.zeros((2, 4), dtype=torch.int64)
    module.layer_multipliers = torch.ones(2, dtype=torch.int64)
    module.ngram_heads_vocab_sizes = torch.tensor([31], dtype=torch.int64)
    module.ngram_heads_offsets = torch.zeros(1, dtype=torch.int64)
    module.ngram_embedding = nn.Embedding(32, 4)
    with torch.no_grad():
        module.ngram_embedding.weight.fill_(1)

    monkeypatch.setattr(amd_ple_layer_module, "is_offload_process", lambda: True)
    output_buffer = torch.empty((4, 4))
    ...
    monkeypatch.setattr(amd_ple_layer_module.envs, "VLLM_PLE_CPU_OFFLOAD", True)
    monkeypatch.setattr(amd_ple_layer_module, "is_offload_process", lambda: False)
    assert module.load_weights([("ngram_embedding.weight", torch.empty(1))]) == set()
