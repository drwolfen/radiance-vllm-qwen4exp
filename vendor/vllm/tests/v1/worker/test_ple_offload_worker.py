...
class _TestStreamingModelLoader:
...
class _FakeHipRuntime:
    ...
    def hipInit(self, flags) -> int:
        self.init_calls.append(flags.value)
        return self.init_status

    def hipHostRegister(self, address, nbytes, flags) -> int:
        self.register_calls.append((address.value, nbytes.value, flags.value))
        return self.register_status

    def hipHostUnregister(self, address) -> int:
        self.unregister_calls.append(address.value)
        return self.unregister_status

    @staticmethod
    def hipGetErrorString(status: int) -> bytes:
        return f"fake HIP error {status}".encode()


def _mapped_qweight_runner(tmp_path, nbytes: int = 4096):
    mmap_path = tmp_path / "ple-qweight.bin"
    mmap_path.write_bytes(bytes(nbytes))
...
def _load_test_ple_weights(
    ...
    loader = _TestStreamingModelLoader(checkpoint_names)
    monkeypatch.setattr(
        ple_offload_worker,
        "initialize_model",
        lambda **_: model,
...
def test_ple_mmap_host_registration_is_default_off(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runner = ple_offload_worker.PleOffloadRunner.__new__(
        ple_offload_worker.PleOffloadRunner
    ...
    runner._layers = {}
    runner._host_registrations = []
    monkeypatch.delenv("VLLM_PLE_MMAP_HOST_REGISTER", raising=False)
    monkeypatch.setattr(
        ple_offload_worker,
        "_hip_host_registration_api",
        lambda: pytest.fail("disabled registration loaded the HIP runtime"),
    )

    runner._maybe_register_file_backed_qweights()

    assert runner._host_registrations == []


def test_ple_mmap_host_registration_owns_exact_mapping_until_close(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    runner, layer = _mapped_qweight_runner(tmp_path)
    runtime = _FakeHipRuntime()
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER", "1")
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER_EXPECTED_BYTES", "4096")
    monkeypatch.setattr(
        ple_offload_worker,
        "_hip_host_registration_api",
        lambda: runtime,
    ...
    assert runtime.register_calls == [(layer.qweight.data_ptr(), 4096, 0)]
    assert runtime.init_calls == [0]
    assert layer._vllm_gguf_mmap_trim_rows == 0
    assert "bytes=4096" in caplog.text
    assert "validated 4096" not in caplog.text

    runner.close()
    runner.close()

    assert runtime.unregister_calls == [layer.qweight.data_ptr()]


def test_ple_mmap_host_registration_fails_closed_on_size_mismatch(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    runner, _ = _mapped_qweight_runner(tmp_path)
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER", "1")
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER_EXPECTED_BYTES", "8192")
    monkeypatch.setattr(
        ple_offload_worker,
        "_hip_host_registration_api",
        lambda: pytest.fail("size mismatch loaded the HIP runtime"),
    )

    with pytest.raises(RuntimeError, match="expected 8192 bytes.*validated 4096"):
        runner._maybe_register_file_backed_qweights()

    assert runner._host_registrations == []


def test_ple_mmap_host_registration_fails_closed_on_hip_error(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    runner, layer = _mapped_qweight_runner(tmp_path)
    runtime = _FakeHipRuntime(register_status=17)
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER", "1")
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER_EXPECTED_BYTES", "4096")
    monkeypatch.setattr(
        ple_offload_worker,
    ...
    with pytest.raises(RuntimeError, match="hipHostRegister failed.*4096 bytes"):
        runner._maybe_register_file_backed_qweights()

    assert runtime.register_calls == [(layer.qweight.data_ptr(), 4096, 0)]
    assert runtime.unregister_calls == []
    assert runner._host_registrations == []


def test_ple_mmap_host_registration_preserves_memory_reserve(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    runner, _ = _mapped_qweight_runner(tmp_path)
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER", "1")
    monkeypatch.setenv("VLLM_PLE_MMAP_HOST_REGISTER_EXPECTED_BYTES", "4096")
    monkeypatch.setenv("VLLM_PLE_PINNED_RESERVE_BYTES", "8192")
    monkeypatch.setattr(
        ple_offload_worker,
        "_proc_mem_available_bytes",
        lambda: 12_287,
    ...
    monkeypatch.setattr(
        ple_offload_worker,
        "_hip_host_registration_api",
        lambda: pytest.fail("memory preflight initialized HIP"),
    )

    with pytest.raises(RuntimeError, match=r"below mapping\+reserve.*12288"):
        runner._maybe_register_file_backed_qweights()
...
def test_ple_offload_wait_only_waits_for_done(
    ...
    wait_flags = SimpleNamespace(CU_STREAM_WAIT_VALUE_EQ=SimpleNamespace(value=1))
    monkeypatch.setattr(ple_offload_layer, "_IS_HIP", False)
    monkeypatch.setattr(ple_offload_layer, "cuda_driver", driver)
    monkeypatch.setattr(ple_offload_layer, "CUstreamWaitValue_flags", wait_flags)
    monkeypatch.setattr(
...
def test_ple_offload_hip_stream_memory_operations(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    wait_calls = []
    write_calls = []
    ...
    runtime = SimpleNamespace(
        hipStreamWaitValue32=lambda *args: wait_calls.append(args) or 0,
        hipStreamWriteValue32=lambda *args: write_calls.append(args) or 0,
    )

    monkeypatch.setattr(ple_offload_layer, "_IS_HIP", True)
    monkeypatch.setattr(ple_offload_layer, "_get_hip_runtime", lambda: runtime)

    ple_offload_layer._stream_wait_value32(stream, 23, 1)
    ple_offload_layer._stream_write_value32(stream, 23, 0)
    ...
    assert wait_calls == [
        (
            ...
            23,
            1,
            ple_offload_layer._HIP_STREAM_WAIT_VALUE_EQ,
            ple_offload_layer._UINT32_MASK,
        )
    ]
    assert write_calls == [(stream.cuda_stream, 23, 0, 0)]
...
@pytest.mark.parametrize(
    ...
    [
        ("Qwen4ExpForCausalLM", False, "cuda", None),
        ("Qwen4ExpForCausalLM", False, "rocm", None),
        ("Qwen4ExpForConditionalGeneration", True, "cuda", None),
        ("UnsupportedArchitecture", True, "cuda", "architecture"),
    ],
)
...
def test_ple_offload_runner_groups_registrations_by_dp_rank(
    ...
    runner._input_bufs = {}
    residency_barrier = []
    runner._maybe_register_file_backed_qweights = lambda: residency_barrier.append(
        {
            "worker_targets": len(runner._worker_targets),
            "input_bufs": len(runner._input_bufs),
            "pinned_bufs": len(runner._pinned_bufs),
        }
    )
...
def test_ple_offload_runner_routes_requests_layer_first(
    ...
    class FakeLayer:
        ...
        def forward_impl(
            ...
            self._vllm_ple_last_timing = {
                "ngram_id_us": 1.0,
                "embedding_gather_dequant_us": 2.0,
                "output_buffer_copy_us": 3.0,
            }
    ...
    assert "ngram_id_us=1.000" in caplog.text
    assert "embedding_gather_dequant_us=2.000" in caplog.text
    assert "output_buffer_copy_us=3.000" in caplog.text
