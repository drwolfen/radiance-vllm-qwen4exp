...
def test_align_draft_multimodal_mask_selects_compacted_rows() -> None:
    mask = torch.tensor([False, True, False, True, False])
    indices = torch.tensor([0, 2, 3], dtype=torch.int64)

    aligned = align_draft_multimodal_mask(mask, indices, expected_tokens=3)

    assert aligned is not None
    assert aligned.tolist() == [False, False, True]


def test_align_draft_multimodal_mask_preserves_matching_mask() -> None:
    mask = torch.tensor([False, True, False])

    assert align_draft_multimodal_mask(mask, None, expected_tokens=3) is mask


def test_align_draft_multimodal_mask_rejects_unexplained_mismatch() -> None:
    mask = torch.tensor([False, True, False])

    with pytest.raises(RuntimeError, match="without compaction indices"):
        align_draft_multimodal_mask(mask, None, expected_tokens=2)
