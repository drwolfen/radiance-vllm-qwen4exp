...
class TestCSALinearPacking:
    ...
    def test_single_rank_mtp_main_kv_is_physically_padded(self):
        specs = _make_csa_linear_specs(num_tuples=2)
        draft_layer = 48
        draft_names = {
            _main_kv_name(1): f"mtp.layers.{draft_layer}.self_attn",
            _compressed_name(1): (
                f"mtp.layers.{draft_layer}.self_attn.indexer.compressed_key_cache"
            ),
            _compressor_state_name(1): (
                f"mtp.layers.{draft_layer}.self_attn.indexer.raw_key_cache"
            ),
        }
        for old_name, new_name in draft_names.items():
            specs[new_name] = specs.pop(old_name)

        draft_main_name = draft_names[_main_kv_name(1)]
        draft_main = specs[draft_main_name]
        assert isinstance(draft_main, FullAttentionSpec)
        specs[draft_main_name] = replace(draft_main, num_kv_heads=4)

        groups = get_kv_cache_groups(_shared_layout_config(), specs)
        compressed_sparse = groups[0].kv_cache_spec
        assert isinstance(compressed_sparse, UniformTypeKVCacheSpecs)
        target_main = compressed_sparse.kv_cache_specs[_main_kv_name(0)]
        padded_draft_main = compressed_sparse.kv_cache_specs[draft_main_name]
        assert isinstance(target_main, FullAttentionSpec)
        assert isinstance(padded_draft_main, FullAttentionSpec)
        assert target_main.num_kv_heads == 2
        assert padded_draft_main.num_kv_heads == 4
        assert target_main.page_size_bytes == padded_draft_main.page_size_bytes
        assert (
            target_main.page_size_padded
            == padded_draft_main.unpadded_page_size_bytes
        )
