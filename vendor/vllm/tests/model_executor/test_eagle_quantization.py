...
def test_get_draft_quant_config_with_draft_load_config():
    mock_draft_model_config = Mock(spec=ModelConfig)
    mock_target_load_config = Mock(spec=LoadConfig)
    mock_draft_load_config = Mock(spec=LoadConfig)
    mock_speculative_config = Mock(spec=SpeculativeConfig)
    mock_speculative_config.draft_model_config = mock_draft_model_config
    mock_speculative_config.draft_load_config = mock_draft_load_config

    mock_vllm_config = Mock(spec=VllmConfig)
    mock_vllm_config.speculative_config = mock_speculative_config
    mock_vllm_config.load_config = mock_target_load_config
    ...
    with patch.object(
        ...
        VllmConfig.get_quantization_config.assert_called_once_with(
            mock_draft_model_config, mock_draft_load_config
        )
        assert result == mock_quant_config


def test_get_draft_quant_config_reuses_prepared_draft_config():
    mock_draft_model_config = Mock(spec=ModelConfig)
    mock_speculative_config = Mock(spec=SpeculativeConfig)
    mock_speculative_config.draft_model_config = mock_draft_model_config
    mock_speculative_config.draft_load_config = Mock(spec=LoadConfig)

    mock_vllm_config = Mock(spec=VllmConfig)
    mock_vllm_config.speculative_config = mock_speculative_config
    mock_vllm_config.model_config = mock_draft_model_config
    mock_vllm_config.load_config = Mock(spec=LoadConfig)
    mock_vllm_config.quant_config = Mock()

    with patch.object(VllmConfig, "get_quantization_config") as get_quant_config:
        result = get_draft_quant_config(mock_vllm_config)

    get_quant_config.assert_not_called()
    assert result is mock_vllm_config.quant_config
