...
from vllm.model_executor.offloader import uva as uva_module
...
@pytest.mark.parametrize(
    ("env_name", "value", "device", "expected_gib"),
    [
        ("R9V_CPU_OFFLOAD_GB_BY_DEVICE", "1.5,2.25", 1, 2.25),
        ("RADIANCE_CPU_OFFLOAD_GB_BY_DEVICE", "3.0,4.0", 0, 3.0),
    ],
)
def test_uva_offloader_device_budget_env(
    monkeypatch, env_name: str, value: str, device: int, expected_gib: float
) -> None:
    monkeypatch.delenv("R9V_CPU_OFFLOAD_GB_BY_DEVICE", raising=False)
    monkeypatch.delenv("RADIANCE_CPU_OFFLOAD_GB_BY_DEVICE", raising=False)
    monkeypatch.setenv(env_name, value)
    monkeypatch.setattr(torch.cuda, "is_available", lambda: True)
    monkeypatch.setattr(torch.cuda, "current_device", lambda: device)
    monkeypatch.setattr(uva_module, "should_pin_memory", lambda: False)
    monkeypatch.setattr(uva_module, "is_uva_available", lambda: False)

    offloader = UVAOffloader(cpu_offload_max_bytes=1)

    assert offloader.cpu_offload_max_bytes == int(expected_gib * 1024**3)


def test_uva_offloader_prefers_r9v_budget_env(monkeypatch) -> None:
    monkeypatch.setenv("R9V_CPU_OFFLOAD_GB_BY_DEVICE", "2")
    monkeypatch.setenv("RADIANCE_CPU_OFFLOAD_GB_BY_DEVICE", "7")
    monkeypatch.setattr(torch.cuda, "is_available", lambda: True)
    ...
    assert offloader.cpu_offload_max_bytes == 2 * 1024**3
        ...
        with vllm_runner(
            ...
            for name in model._tower_model_names:
                ...
                if disable_uva:
                    ...
                    # Accelerator UVA views do not own the host allocation.
                    # Every offloaded parameter must retain its pinned tensor.
                    assert all(
                        hasattr(p, "_vllm_uva_cpu_data") for p in params
                    )
