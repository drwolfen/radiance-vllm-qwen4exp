...
class SharedExperts(torch.nn.Module):
    ...
    @property
    def pending_output(self) -> torch.Tensor:
        """Return the current output without consuming it."""

        output = self._output[self._output_idx]
        assert output is not None
        return output

    def run_synchronously(self, shared_experts_input: torch.Tensor) -> None:
        """Run shared experts on the current stream regardless of overlap policy."""
