...
class MoERunner(MoERunnerInterface):
    ...
    def _maybe_apply_shared_experts(
        ...
        if self._shared_experts is not None:
            ...
            if self._quant_method.fuses_shared_expert_output:
                if order == SharedExpertsOrder.NO_OVERLAP:
                    self._shared_experts.run_synchronously(shared_experts_input)
                return
    ...
    def forward(
        ...
        fuses_shared_output = (
            self._quant_method.fuses_shared_expert_output
            and self._shared_experts is not None
        )
        if fuses_shared_output:
            if self.routed_scaling_factor != 1.0:
                raise RuntimeError(
                    "Fused MoE shared epilogue requires routed_scaling_factor=1"
                )
            if self.routed_output_transform is not None:
                raise RuntimeError(
                    "Fused MoE shared epilogue does not support output transforms"
                )
            if self._fused_output_is_reduced:
                raise RuntimeError(
                    "Fused MoE shared epilogue requires an unreduced routed output"
                )
            if og_hidden_dim_pre_xform is not None:
                raise RuntimeError(
                    "Fused MoE shared epilogue does not support padded outputs"
                )
        ...
        if fuses_shared_output:
            if shared_output is None:
                raise RuntimeError("Fused MoE shared epilogue lost its shared output")
            result = fused_output
        elif shared_output is not None:
