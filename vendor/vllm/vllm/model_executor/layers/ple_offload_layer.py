...
handled by :class:`CpuGpuSemaphore`, which uses CUDA or HIP stream memory
operations.
...
@functools.cache
def _get_hip_runtime() -> ctypes.CDLL:
    library = ctypes.util.find_library("amdhip64")
    if library is None:
        rocm_path = os.environ.get("ROCM_PATH", "/opt/rocm")
        absolute_library = os.path.join(rocm_path, "lib", "libamdhip64.so")
        library = (
            absolute_library
            if os.path.isfile(absolute_library)
            else "libamdhip64.so"
        )
    runtime = ctypes.CDLL(library)
    runtime.hipStreamWaitValue32.argtypes = (
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_uint32,
        ctypes.c_uint,
    ...
    runtime.hipStreamWaitValue32.restype = ctypes.c_int
    runtime.hipStreamWriteValue32.argtypes = (
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_uint32,
        ctypes.c_uint,
    )
    runtime.hipStreamWriteValue32.restype = ctypes.c_int
    return runtime


def _hip_check(result: int, operation: str) -> None:
    if result != 0:
        raise RuntimeError(f"{operation} failed with hipError_t={result}")


def _stream_write_value32(
    stream: torch.cuda.Stream,
    pointer: int,
    value: int,
) -> None:
    if _IS_HIP:
        runtime = _get_hip_runtime()
        _hip_check(
            runtime.hipStreamWriteValue32(
                stream.cuda_stream,
                pointer,
                value,
                0,
    ...
    _cuda_check(
        cuda_driver.cuStreamWriteValue32(
            ...
            cuda_driver.CUdeviceptr(pointer),
            value,
            0,
        ),
...
def _stream_wait_value32(
    ...
    assert cuda_driver is not None
    assert CUstreamWaitValue_flags is not None
    _cuda_check(
...
class CpuGpuSemaphore:
    """Cross-process semaphore backed by a one-element int32 GPU tensor.

    The flag is stored in a regular GPU tensor that is shared through
    PyTorch IPC. Both processes obtain device pointers that map to the same
    physical GPU memory, so a stream-memory wait in the GPU worker observes the
    write issued by the offload process.
    """
    ...
    def from_ipc_tensor(cls, flag_tensor: torch.Tensor) -> "CpuGpuSemaphore":
        """Construct a semaphore from a GPU tensor received through IPC."""
    ...
    def flag_tensor(self) -> torch.Tensor:
        """Return the GPU tensor used to share the semaphore through IPC."""
...
# ---------------------------------------------------------------------------
# Wraps a stream memory wait as a torch custom op so that
# torch.compile(fullgraph=True) treats the driver call as an opaque node.
