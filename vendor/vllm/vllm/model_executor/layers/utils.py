...
def _qwen38_hc_down_hip():
    """Load the explicit opt-in Qwen4Exp HC-down gfx1201 extension."""
    global _QWEN38_HC_DOWN_HIP
    if _QWEN38_HC_DOWN_HIP is not None:
        return _QWEN38_HC_DOWN_HIP
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot load Qwen4Exp HC-down extension from {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    _QWEN38_HC_DOWN_HIP = module
    return module
...
def _use_qwen38_hc_down_bf16_m3(
    x: torch.Tensor,
    weight: torch.Tensor,
    bias: torch.Tensor | None,
) -> bool:
    """Strict default-off gate for the measured target-verifier geometry."""
    return (
        os.environ.get("QWEN38_USE_DENSE_HC_DOWN_BF16_M3") == "1"
        and bias is None
        and x.ndim == 2
        and tuple(x.shape) == (3, 10240)
        and weight.ndim == 2
        and tuple(weight.shape) == (336, 10240)
...
def rocm_unquantized_gemm_impl(
    ...
    # This exact BF16 path is numerically identical to wvSplitK for Qwen4Exp's
    # merged [320 down + 4 injection + 12 padding] projection.  It is kept
    # before generic skinny-GEMM selection so the opt-in remains visible to
    # both eager execution and CUDA-graph capture.
    if _use_qwen38_hc_down_bf16_m3(x, weight, bias):
        return _qwen38_hc_down_hip().hc_down_bf16_m3(weight, x, 0)
