...
if GDN_AITER_TRITON_AVAILABLE:
    try:
        from aiter.ops.triton.conv.causal_conv1d_update_single_token import (
...
def _diagnostic_gdn_sync(prefix: str, stage: str) -> None:
    if os.environ.get("VLLM_QWEN_GDN_DIAGNOSTIC_SYNC", "0") != "1":
        return
    logger.warning("GDN diagnostic: %s %s begin", prefix, stage)
    torch.accelerator.synchronize()
    logger.warning("GDN diagnostic: %s %s end", prefix, stage)
...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        from . import fused_gdn_mtp_hip

        self.enable_hip_fused_gdn_mtp = fused_gdn_mtp_hip.enabled()
        fused_gdn_mtp_hip.preflight()
        logger.info_once("GDN decode kernel: %s", self.gdn_decode_kernel)
        if self.enable_hip_fused_gdn_mtp:
            logger.info_once("Qwen3.8 TP2 fused speculative GDN HIP kernel enabled")
    ...
    def forward_hip(
        ...
        if GDN_AITER_TRITON_AVAILABLE:
            ...
            _diagnostic_gdn_sync(self.prefix, "core-op")

            output = self._output_projection(core_attn_out, z)
            _diagnostic_gdn_sync(self.prefix, "output-projection")
            return output
        else:
    ...
    def _forward_core_rocm(
        ...
        ``_forward_core_decode_aiter`` when AITER supports the input layout.
        Otherwise unpacks the packed layout and falls through to
        ``_forward_core``.
        ...
        # Old AITER builds only understand Qwen3-Next's interleaved packing.
        # Newer builds accept an explicit layout and also cover Qwen3.8's flat
        # q/k/v/z tensors.
        if (
    ...
    def _forward_core(
        ...
        _diagnostic_gdn_sync(self.prefix, "core-entry")
        ...
        if spec_sequence_masks is not None:
            ...
            _diagnostic_gdn_sync(self.prefix, "spec-conv")
        ...
        if attn_metadata.num_prefills > 0:
            ...
            _diagnostic_gdn_sync(self.prefix, "prefill-conv")
        ...
        if self._can_use_hip_fused_gdn_mtp_core(attn_metadata):
            assert mixed_qkv_spec is not None
            assert a_spec is not None
            assert b_spec is not None
            self._forward_core_decode_spec_post_conv_hip(
        ...
        if spec_sequence_masks is not None:
            _diagnostic_gdn_sync(self.prefix, "spec-recurrent-entry")
            ...
            _diagnostic_gdn_sync(self.prefix, "spec-recurrent")
        else:
        ...
        if attn_metadata.num_prefills > 0:
            ...
            _diagnostic_gdn_sync(self.prefix, "prefill-recurrent")
            # Init cache
            ssm_state[prefill_state_indices] = last_recurrent_state.to(ssm_state.dtype)
            _diagnostic_gdn_sync(self.prefix, "prefill-state-write")
    ...
    def _can_use_hip_fused_gdn_mtp_core(
        ...
        return (
            ...
            and fused_gdn_mtp_hip.core_supported(self)
        )

    def _forward_core_decode_spec_post_conv_hip(
        self,
        mixed_qkv: torch.Tensor,
        b: torch.Tensor,
        a: torch.Tensor,
    ...
    def _forward_core_decode_spec_post_conv_fused_norm(
        ...
        if fused_gdn_mtp_hip.supported(self):
            fused_gdn_mtp_hip.run(
                self,
                mixed_qkv=mixed_qkv,
                a=a,
                b=b,
    ...
    def _can_use_fused_gdn_mtp_decode(
        ...
        implementation_available = fused_gdn_mtp_hip.supported(self) or (
