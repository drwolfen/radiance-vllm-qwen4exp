...
class UVAOffloader(BaseOffloader):
    ...
    def __init__(
        ...
        budget_env = "R9V_CPU_OFFLOAD_GB_BY_DEVICE"
        per_device_gb = os.environ.get(budget_env)
        if per_device_gb is None:
            budget_env = "RADIANCE_CPU_OFFLOAD_GB_BY_DEVICE"
            per_device_gb = os.environ.get(budget_env)
        if per_device_gb and torch.cuda.is_available():
            budgets = [value.strip() for value in per_device_gb.split(",")]
            device_index = torch.cuda.current_device()
            if device_index >= len(budgets):
                raise ValueError(
                    f"{budget_env} does not provide a "
                    f"budget for accelerator device {device_index}: {per_device_gb!r}"
                )
            try:
                selected_gb = float(budgets[device_index])
            except ValueError as exc:
                raise ValueError(
                    f"{budget_env} must be a comma-separated "
                    f"list of non-negative GiB values, got {per_device_gb!r}"
                ) from exc
            if selected_gb < 0:
                raise ValueError(
                    f"{budget_env} values must be non-negative, "
                    f"got {per_device_gb!r}"
                )
            cpu_offload_max_bytes = int(selected_gb * 1024**3)
            logger.info(
                "Using device-specific CPU offload budget %.3g GiB for device %d.",
                selected_gb,
                device_index,
            )
    ...
    def _maybe_offload_to_cpu(self, module: nn.Module, prefix: str = "") -> nn.Module:
        ...
        for name, p in module.named_parameters():
            ...
            # GGUF creates placeholder parameters and materializes their packed
            # storage while loading. Reserve the budget from the logical tensor
            # shape now, and let the GGUF loader allocate the eventual storage
            # directly in pinned CPU/UVA memory. Treating these placeholders as
            # zero bytes silently bypasses cpu_offload_gb and later OOMs in the
            # checkpoint loader.
            if isinstance(p, UninitializedParameter) and hasattr(p, "tensor_shape"):
                logical_bytes = prod(p.tensor_shape) * torch.tensor(
                    [], dtype=torch.get_default_dtype(), device="cpu"
                ).element_size()
                remaining = self.cpu_offload_max_bytes - self.cpu_offload_bytes
                if logical_bytes > remaining:
                    continue
                p._vllm_is_uva_offloaded = True
                p._vllm_uva_pin_memory = self.pin_memory
                self.cpu_offload_bytes += logical_bytes
            ...
            if not self.uva_offloading:
                ...
                # The accelerator view aliases the host allocation but does not
                # keep the originating CPU tensor alive.  Retain it explicitly;
                # otherwise the pinned allocation can be released after this
                # function returns and a later kernel faults on the stale UVA
                # address.
                p._vllm_uva_cpu_data = cpu_data
                p._vllm_is_uva_offloaded = True
