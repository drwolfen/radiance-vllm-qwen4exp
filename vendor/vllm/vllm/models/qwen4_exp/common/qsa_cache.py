...
class QSAStateBackend(AttentionBackend):
    ...
    def supported_kv_cache_layouts(cls) -> tuple[KVCacheLayout, ...]:
        # Qwen4Exp mixes QSA side-state pages with much larger main KV pages,
        # which requires a block-outer layout.  The binder below exposes a
        # canonical [B, N, H, C] view to the QSA kernels.
...
class _QSAStateCache(nn.Module, AttentionLayerBase):
    ...
    @staticmethod
    def _canonicalize_kv_cache(kv_cache: torch.Tensor) -> torch.Tensor:
        if kv_cache.ndim != 4:
            raise ValueError("QSA cache must be four-dimensional")
        if kv_cache.shape[2] == 1:
            # LBNHC per-layer view: [B, N, H, C].
            return kv_cache
        if kv_cache.shape[1] == 1:
            # BLHNC per-layer view: [B, H, N, C].
            return kv_cache.transpose(1, 2)
        raise ValueError("QSA cache must contain exactly one KV head")

    def bind_kv_cache(self, kv_cache: torch.Tensor) -> None:
        super().bind_kv_cache(self._canonicalize_kv_cache(kv_cache))
...
class QSAKeyStateCache(_QSAStateCache):
    ...
    def bind_kv_cache(self, kv_cache: torch.Tensor) -> None:
        kv_cache = self._canonicalize_kv_cache(kv_cache)
