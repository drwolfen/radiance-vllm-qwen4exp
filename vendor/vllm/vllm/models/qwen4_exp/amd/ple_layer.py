...
class Qwen4ExpNGramEmbedding(PleOffloadLayer):
    ...
    def forward_impl(  # type: ignore[override]
        ...
        detailed_timing = (
            offload_process and os.environ.get(_PLE_WORKER_TIMING_ENV, "0") == "1"
        )
        ngram_id_started_ns = time.perf_counter_ns() if detailed_timing else 0
        if detailed_timing:
        ...
        if offload_process:
            ngram_id_us = (
                (time.perf_counter_ns() - ngram_id_started_ns) / 1_000
                if detailed_timing
                else 0.0
            )
            embedding_started_ns = time.perf_counter_ns() if detailed_timing else 0
            output = self.ngram_embedding(ngram_ids).flatten(-2)
            embedding_us = (
                (time.perf_counter_ns() - embedding_started_ns) / 1_000
                if detailed_timing
                else 0.0
            )
            output_copy_us = 0.0
            if output_buffer is not None:
                destination = output_buffer[:num_tokens, : self.embedding_dim]
                output_copy_started_ns = (
                    time.perf_counter_ns() if detailed_timing else 0
                )
                destination.copy_(output)
                output_copy_us = (
                    (time.perf_counter_ns() - output_copy_started_ns) / 1_000
                    if detailed_timing
                    else 0.0
                )
                output = destination
            if detailed_timing:
                self._vllm_ple_last_timing = {
                    "ngram_id_us": ngram_id_us,
                    "embedding_gather_dequant_us": embedding_us,
                    "output_buffer_copy_us": output_copy_us,
                }
            return output
