...
def _try_fused_hc_up_mix(
    up_projection: nn.Module,
    raw_lora: torch.Tensor,
    xn: torch.Tensor,
    hc_count: int,
) -> torch.Tensor | None:
    quant_method = getattr(up_projection, "quant_method", None)
    apply_hc_up_mix = getattr(quant_method, "apply_hc_up_mix", None)
    if apply_hc_up_mix is None:
        return None
    return apply_hc_up_mix(up_projection, raw_lora, xn, hc_count)
...
class GatedResidual(nn.Module):
    ...
    def mix(
        ...
        block_input = _try_fused_hc_up_mix(
