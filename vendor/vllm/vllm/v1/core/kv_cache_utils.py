...
def _get_kv_cache_groups_csa_linear(
    ...
    if any(
        cache.main_kv[1].num_kv_heads != expected_local_kv_heads
        and not cache.main_kv[0].startswith("mtp.")
        for cache in tuples
    ):
        raise ValueError(
    ...
    # A single-rank MTP draft can retain all KV heads while the TP-sharded
    # target owns fewer heads per rank. CSA aliases one rectangular main-KV
    # page per full-attention layer, so pad the smaller target pages to the
    # largest draft/target page without changing either layer's logical head
    # shape. The cache view builder observes ``num_kv_heads`` for the logical
    # view and ``page_size_padded`` only for its physical stride.
    main_kv_page = max(cache.main_kv[1].page_size_bytes for cache in tuples)
    if any(cache.main_kv[1].page_size_bytes != main_kv_page for cache in tuples):
        for name, spec in tuple(roles.main_kv.items()):
            if spec.page_size_bytes != main_kv_page:
                roles.main_kv[name] = replace(spec, page_size_padded=main_kv_page)
        tuples = _get_csa_linear_cache_tuples(roles)
    ...
    for cache in tuples:
        ...
        if not (
            ...
            and compressed.page_size_padded is None
            and compressor_state.page_size_padded is None
        ):
            raise ValueError(
