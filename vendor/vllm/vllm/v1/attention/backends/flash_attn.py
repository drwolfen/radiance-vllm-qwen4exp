...
if is_flash_attn_varlen_func_available():
    ...
    # Qwen4Exp's QSA reuses FlashAttentionImpl for its cache update even when
    # the optional flash_attn_varlen_func is unavailable.  The ROCm cache-write
    # op is provided independently by vLLM's custom ops, so keep it importable
    # for that path instead of coupling it to the varlen-attention probe.
