...
def _ple_diagnostic(message: str, *args: object) -> None:
    if os.environ.get("VLLM_PLE_DIAGNOSTIC", "0") == "1":
        logger.warning("PLE diagnostic: " + message, *args)


def _cuda_check(result: Any, operation: str) -> Any:
    """Check a CUDA-driver or PyTorch CUDA/HIP-runtime result."""
...
def _host_register(pointer: int, size: int) -> None:
    """Page-lock shared host storage without replacing its allocation."""
    if _IS_HIP:
        # PyTorch's ROCm build exposes the HIP runtime through its CUDA-
        # compatible cudart shim.  Using that shim avoids requiring the
        # NVIDIA-only cuda-python driver bindings in a ROCm worker.
        _cuda_check(
            torch.cuda.cudart().cudaHostRegister(pointer, size, 1),
            "hipHostRegister(PLE input buffer)",
        )
        return
...
def _host_unregister(pointer: int) -> None:
    """Release a host registration created by :func:`_host_register`."""
    if _IS_HIP:
        _cuda_check(
            torch.cuda.cudart().cudaHostUnregister(pointer),
            "hipHostUnregister(PLE input buffer)",
        )
        return

    assert cuda_driver is not None
    _cuda_check(
        cuda_driver.cuMemHostUnregister(pointer),
...
class PleOffloadConnector:
    ...
    def _unpin_input_buffers(self) -> None:
        ...
        for buffer in reversed(self._pinned_input_buffers):
            try:
                _host_unregister(buffer.data_ptr())
            except RuntimeError:
    ...
    def _process_request(self, request: PleOffloadRequest, socket: zmq.Socket) -> None:
        ...
        _ple_diagnostic(
            "connector request start dp=%d tokens=%d reqs=%d cuda_inputs=%s",
            request.dp_rank,
            request.num_tokens,
            request.num_reqs,
        ...
        _ple_diagnostic("connector inputs staged dp=%d", request.dp_rank)

        with torch.cuda.nvtx.range("ple_offload.send_request"):
            socket.send(msgspec.msgpack.encode(request))
        _ple_diagnostic("connector request sent dp=%d", request.dp_rank)
    ...
    def _launch(
        ...
        _ple_diagnostic(
            "connector request queued dp=%d tokens=%d reqs=%d",
            request.dp_rank,
            request.num_tokens,
            request.num_reqs,
        )
