...
class Worker(WorkerBase):
    ...
    def _has_ple_layers(self) -> bool:
        ...
        Qwen4Exp's PLE layer is an ordinary decoder-layer child.  Under PP it
        exists only on the stage owning its layer index, so later stages must
        not register empty connector sets with the CPU offload worker.
        """
        if not envs.VLLM_PLE_CPU_OFFLOAD:
            return False
        text_config = self.model_config.hf_text_config
        ple_layer_ids = tuple(getattr(text_config, "ple_layer_ids", ()) or ())
        ...
        # TP ranks are contiguous within each PP stage.
    ...
    def _validate_ple_offload_config(self) -> None:
        ...
        pp_size = parallel_config.pipeline_parallel_size
        if pp_size > 1:
            # One shared worker can serve PLE on the first stage without
            # involving later stages.  Reject cross-stage PLE layouts until
            # the request protocol carries a PP-stage identity.
            from vllm.distributed.utils import get_pp_indices
            ...
            ple_layer_ids = tuple(
                getattr(text_config, "ple_layer_ids", ()) or ()
            )
            if not ple_layer_ids or any(
                layer_id >= first_stage_end for layer_id in ple_layer_ids
            ):
                unsupported.append(f"PP={pp_size} with cross-stage PLE")
