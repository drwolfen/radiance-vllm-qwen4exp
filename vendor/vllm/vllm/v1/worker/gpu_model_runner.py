...
def align_draft_multimodal_mask(
    mask: torch.Tensor | None,
    token_indices: torch.Tensor | None,
    expected_tokens: int,
) -> torch.Tensor | None:
    """Match a multimodal marker mask to compacted draft-token rows.

    The target runner may remove rejected speculative rows before invoking the
    draft model.  A marker mask built for the scheduled target rows must follow
    that same selection or it no longer indexes the draft embedding buffer.
    """
    if mask is None or mask.shape[0] == expected_tokens:
        return mask
    if token_indices is None:
        raise RuntimeError(
            "multimodal marker count differs from draft rows without compaction "
            "indices"
        )
    aligned = torch.index_select(mask, 0, token_indices.to(device=mask.device))
    if aligned.shape[0] != expected_tokens:
        raise RuntimeError(
            "compacted multimodal marker count does not match draft rows: "
            f"{aligned.shape[0]} != {expected_tokens}"
        )
    return aligned
    ...
    def propose_draft_token_ids(
        ...
        if spec_config.method == "ngram":
            ...
            if self.supports_mm_inputs and self.drafter.supports_mm_inputs:
                ...
                is_mm_embed = align_draft_multimodal_mask(
                    is_mm_embed,
                    token_indices,
