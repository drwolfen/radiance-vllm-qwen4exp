...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def _bind_speculator_model_state(self) -> None:
        if isinstance(self.speculator, DraftModelSpeculator):
            self.speculator.bind_model_state(self.model_state)
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        # profile_run() proposes drafts before KV-cache initialization calls
        # set_attn(). Bind target-derived state (for example M-RoPE positions)
        # as soon as the model state exists.
        self._bind_speculator_model_state()
    ...
    def execute_model(
        ...
        # Run model. Keep this scope immediately around the actual target call:
        # Kineto can then associate asynchronous HIP kernels through their
        # external IDs without charging V2 input preparation to target compute.
    ...
    def sample_tokens(
        ...
        with record_function_or_nullcontext("gpu_model_runner: postprocess"):
            ...
            # Start async output copy here so that it can overlap with proposal.
