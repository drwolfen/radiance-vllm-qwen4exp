...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # InputBuffers.positions contains logical 1-D positions used by the
        # scheduler, slot mapping, and sampling.  M-RoPE draft models need the
        # target model's exact 3-axis positions in addition to that logical
        # buffer.  Keep a separate, graph-stable buffer just like the legacy
        # GPUModelRunner proposer did.  The extra column intentionally makes
        # slices non-contiguous for torch.compile (see RopeState).
        self.uses_mrope = bool(
            getattr(self.draft_model_config, "uses_mrope", False)
        )
    ...
    def bind_model_state(self, model_state: ModelState) -> None:
        super().bind_model_state(model_state)
        if self.uses_mrope:
            rope_state = getattr(model_state, "rope_state", None)
            if rope_state is None or rope_state.num_dims != 3:
                raise ValueError(
                    "An M-RoPE draft model requires the target model's "
                    "3-axis RopeState"
                )

    def _get_target_mrope_positions(self, num_tokens: int) -> torch.Tensor | None:
        ...
        if rope_state is None or rope_state.num_dims != 3:
            raise RuntimeError(
                "M-RoPE draft positions are unavailable: the target model state "
                "must be bound before profile/propose"
            )
        positions = rope_state.get_positions(num_tokens)
        if positions.ndim != 2 or positions.shape != (3, num_tokens):
            raise RuntimeError(
                "Target M-RoPE positions must have shape "
                f"(3, {num_tokens}), got {tuple(positions.shape)}"
            )
        return positions

    def _get_draft_positions(self, num_tokens: int) -> torch.Tensor:
        if self.uses_mrope:
            assert self.mrope_positions is not None
            return self.mrope_positions[:, :num_tokens]
        return self.input_buffers.positions[:num_tokens]

    def _compact_mrope_positions(
        self, last_token_indices: torch.Tensor, num_reqs: int
    ) -> None:
        if not self.uses_mrope:
            return
        assert self.mrope_positions is not None
        selected = self.mrope_positions[:, last_token_indices]
        self.mrope_positions[:, :num_reqs].copy_(selected)
    ...
    def init_cudagraph_manager(self, cudagraph_mode: CUDAGraphMode) -> None:
        ...
        if speculative_config is not None and speculative_config.enforce_eager:
            logger.info_once(
                "Draft-only eager mode is enabled; target CUDA graphs remain "
                "unchanged."
            )
    ...
    def propose(
        ...
        prepare_prefill_inputs(
            ...
            target_mrope_positions=self._get_target_mrope_positions(
                num_tokens_padded
            ),
        )
    ...
    def _run_model(
        ...
        with set_forward_context(
            ...
            model_inputs = dict(
                ...
                positions=self._get_draft_positions(num_tokens),
    ...
    def _prefill(
        ...
        self._compact_mrope_positions(last_token_indices, num_reqs)
...
@triton.jit
def _prepare_prefill_mrope_positions_kernel(
    draft_positions_ptr,
    draft_positions_stride,
    target_positions_ptr,
    target_positions_stride,
    num_rejected_ptr,
    query_start_loc_ptr,
    BLOCK_SIZE: tl.constexpr,
    NUM_MROPE_DIMS: tl.constexpr,
):
    req_idx = tl.program_id(0)
    query_start = tl.load(query_start_loc_ptr + req_idx)
    query_end = tl.load(query_start_loc_ptr + req_idx + 1)
    query_len = query_end - query_start - tl.load(num_rejected_ptr + req_idx)

    for i in range(0, query_len, BLOCK_SIZE):
        block = i + tl.arange(0, BLOCK_SIZE)
        mask = block < query_len
        for dim in tl.static_range(NUM_MROPE_DIMS):
            target_pos = tl.load(
                target_positions_ptr
                + dim * target_positions_stride
                + query_start
                + block,
                mask=mask,
            )
...
def prepare_prefill_inputs(
    ...
    target_mrope_positions: torch.Tensor | None = None,
) -> torch.Tensor:
    num_reqs = input_batch.num_reqs
    if (draft_mrope_positions is None) != (target_mrope_positions is None):
        raise ValueError(
            "draft and target M-RoPE position buffers must be provided together"
        )
    if draft_mrope_positions is not None:
        assert target_mrope_positions is not None
        if (
            draft_mrope_positions.ndim != 2
            or target_mrope_positions.ndim != 2
            or draft_mrope_positions.shape[0] != 3
            or target_mrope_positions.shape[0] != 3
        ):
            raise ValueError("M-RoPE position buffers must have shape [3, tokens]")
        if (
            draft_mrope_positions.dtype != torch.int64
            or target_mrope_positions.dtype != torch.int64
        ):
            raise TypeError("M-RoPE position buffers must use int64")
        if draft_mrope_positions.device != target_mrope_positions.device:
            raise ValueError("M-RoPE position buffers must be on the same device")
        num_tokens = input_batch.num_tokens_after_padding
        if (
            draft_mrope_positions.shape[1] < num_tokens
            or target_mrope_positions.shape[1] < num_tokens
        ):
            raise ValueError(
                f"M-RoPE position buffers must hold at least {num_tokens} tokens"
            )
    ...
    if draft_mrope_positions is not None:
        assert target_mrope_positions is not None
        _prepare_prefill_mrope_positions_kernel[(num_reqs,)](
            draft_mrope_positions,
            draft_mrope_positions.stride(0),
            target_mrope_positions,
            target_mrope_positions.stride(0),
            num_rejected,
            input_batch.query_start_loc,
            BLOCK_SIZE=1024,
            NUM_MROPE_DIMS=3,
...
def _prepare_decode_inputs_kernel(
    ...
    if ADVANCE_DRAFT_POSITIONS:
        ...
        if NUM_MROPE_DIMS > 0:
            for dim in tl.static_range(NUM_MROPE_DIMS):
                mrope_position = tl.load(
                    mrope_positions_ptr
                    + dim * mrope_positions_stride
                    + req_idx
                )
                mrope_position = tl.minimum(
                    mrope_position + 1, max_model_len - 1
                )
                tl.store(
                    mrope_positions_ptr
                    + dim * mrope_positions_stride
...
def prepare_decode_inputs(
    ...
    mrope_positions_arg = (
        input_buffers.positions if mrope_positions is None else mrope_positions
    )
    num_mrope_dims = 0 if mrope_positions is None else mrope_positions.shape[0]
