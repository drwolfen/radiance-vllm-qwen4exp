...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Token and multimodal input embeddings retain the draft model's base
        # embedding width even when hidden-state residuals are HC-multiplexed.
    ...
    def bind_model_state(self, model_state: ModelState) -> None:
        """Bind target model state before profiling or attention setup.

        Memory profiling runs the speculator before KV-cache initialization,
        while ``set_attn`` is called only during KV-cache initialization.  Any
        draft input derived from target model state must therefore use this
        earlier lifecycle hook rather than relying on ``set_attn``.
        """
    ...
    def set_attn(
        ...
        self.bind_model_state(model_state)
