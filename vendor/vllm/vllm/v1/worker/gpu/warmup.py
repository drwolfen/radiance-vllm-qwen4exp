...
def warmup_kernels(
    ...
    model_runner.kv_connector.set_disabled(True)
    logger.info("V2 scheduler warmup: prefill begin")
    worker_execute_model(prefill_output)
    logger.info("V2 scheduler warmup: prefill end; sampler begin")
    ...
    if not model_runner.is_pooling_model:
        ...
        logger.info("V2 scheduler warmup: prefill sampler end")
        ...
        def _run_decode_step(indices: list[int], spec_flags: list[bool]) -> None:
            ...
            logger.info(
                "V2 scheduler warmup: decode begin requests=%s speculative=%s",
                indices,
                spec_flags,
            )
            worker_execute_model(decode_output)
            logger.info("V2 scheduler warmup: decode model end; sampler begin")
            worker_sample_tokens(None)
            logger.info("V2 scheduler warmup: decode sampler end")
    ...
    logger.info("V2 scheduler warmup: cleanup begin")
    worker_execute_model(cleanup_output)
    model_runner.kv_connector.set_disabled(False)
    torch.accelerator.synchronize()
    logger.info("V2 scheduler warmup: complete")
